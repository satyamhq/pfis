...
export const LanguageSelector: React.FC<LanguageSelectorProps> = ({
  ...
  return (
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef}>
      ...
      <button
        ...
        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all border shadow-xs ${
          compact
            ...
            : 'bg-slate-50/90 dark:bg-slate-800/90 text-slate-700 dark:text-slate-200 border-slate-200/80 dark:border-slate-700/80 hover:bg-white dark:hover:bg-slate-750 hover:border-teal-400 dark:hover:border-teal-500'
        }`}
        title="Choose Language"
      >
        <div className="w-5 h-5 rounded-lg bg-teal-500/10 dark:bg-teal-400/10 flex items-center justify-center text-teal-600 dark:text-teal-400">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/90 dark:border-slate-800/90 shadow-2xl z-50 py-2 max-h-96 overflow-y-auto">
          <div className="px-3.5 py-2 border-b border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
            ...
            <span className="text-[10px] font-semibold bg-teal-50 dark:bg-teal-950 text-teal-600 dark:text-teal-300 px-1.5 py-0.5 rounded-md">
              11 Indic
            </span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 text-xs rounded-xl font-medium transition-all ${
                    isSelected
                      ? 'bg-teal-50 text-teal-800 font-bold dark:bg-teal-950/60 dark:text-teal-300 shadow-xs'
